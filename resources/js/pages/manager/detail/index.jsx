...
export default function ManagerDetailOrder() {
    ...
    if (!order) {
        return (
            <DashboardLayout title="Detail Order">
                <div className="py-8 text-center">Data order tidak ditemukan.</div>
            </DashboardLayout>
        );
    }
    ...
    const normalizedSample = selectedSample
        ? {
              ...
              parameter: selectedSample.n_parameter_methods?.test_parameters ?? null,
    ...
    return (
        <DashboardLayout title="Detail Order" header="Detail Order">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <ClientInfoCard client={order.client ?? order.clients ?? { user: {} }} />
                ...
                {normalizedSample && (
                    <>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-6">
                                ...
                                {normalizedSample.parameter ? (
                                    <ParameterInfoCard parameter={normalizedSample.parameter} />
                                ) : (
                                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700">
                                        Data parameter tidak tersedia untuk sample ini.
                                    </div>
                                )}

                                {normalizedSample.method ? (
                                    <MethodInfoCard method={normalizedSample.method} />
                                ) : (
                                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700">
                                        Data metode tidak tersedia untuk sample ini.
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <EquipmentCard equipments={normalizedSample.equipments} />
