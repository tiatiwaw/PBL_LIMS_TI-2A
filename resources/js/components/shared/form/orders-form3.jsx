      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-2xl">
        ...
        <div>
          ...
          <div className="px-4 py-2 space-y-2 mb-6">
            <DetailRow label="Nomor Order" value={dataOrder.nomorOrder} />
            <DetailRow label="Judul Order" value={dataOrder.judulOrder} />
            <DetailRow label="Metode Analisis" value={dataOrder.metodeAnalisis} />
            <DetailRow label="Tipe Order" value={dataOrder.tipeOrder} />
          </div>
          ...
          <div className="mb-6">
            ...
            {samples.map((sample, index) => (
              <div 
                key={sample.id} 
                className={`grid grid-cols-5 p-3 text-sm border-b ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}
              >
                <div className="col-span-1 text-gray-600">{sample.id}</div>
                <div className="col-span-1 font-medium text-gray-800">{sample.name}</div>
                <div className="col-span-1">{sample.bentuk}</div>
                <div className="col-span-1">{sample.kategori}</div>
                <div
          ...
          <div className="px-4 py-2 space-y-2">
            <DetailRow label="Tanggal Masuk" value={dataOrder.tanggalOrder} />
            <DetailRow label="Estimasi Selesai" value={dataOrder.estimasiSelesai} />
          </div>
        </div>
      </div>
    </div>
